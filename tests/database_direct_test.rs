...
async fn test_database_direct_operations() {
    ...
    for (i, metric) in initial_metrics.iter().enumerate() {
        println!("   Node {}: {} (last seen: {})", i+1, metric.address, metric.last_seen);
    }
    ...
    match db.record_successful_check(test_address, Some(70018), Some("Test".to_string()), 30).await {
    ...
    if updated_metrics.iter().any(|m| m.address == test_address) {
        ...
        println!("   30 days ago: {}", chrono::Utc::now() - chrono::Duration::days(30));
    }
}
...
async fn test_database_time_window() {
    ...
    db.record_successful_check(test_address, Some(70018), Some("TimeTest".to_string()), 30).await.unwrap();
